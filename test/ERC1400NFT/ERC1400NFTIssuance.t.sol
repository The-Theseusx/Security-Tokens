...
abstract contract ERC1400NFTIssuanceTest is ERC1400NFTBaseTest {
    ...
    function testShouldFailWhenIssuingNotByIssuer() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                AccessControlUnauthorizedAccount.selector, address(this), ERC1400NFTMockToken.ERC1400_NFT_ISSUER_ROLE()
            )
        );
    ...
    function testShouldNotIssueByPartitionToNonERC1400NFTReceiverImplementer() public {
        ...
        ERC1400NFTMockToken.issueByPartition(
            SHARED_SPACES_PARTITION, address(nonERC1400NFTReceivableContract), newTokenId, ""
        );
        vm.stopPrank();
    }
    ...
    function testIssueByPartitionToERC1400NFTReceiverImplementer() public {
        ...
        emit IssuedByPartition(
            SHARED_SPACES_PARTITION, tokenIssuer, address(ERC1400NFTReceivableContract), newTokenId, ""
        );

        ERC1400NFTMockToken.issueByPartition(
            SHARED_SPACES_PARTITION, address(ERC1400NFTReceivableContract), newTokenId, ""
        );
