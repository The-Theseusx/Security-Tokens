...
contract SigUtils {
	bytes32 internal immutable _domainSeparator; ///@dev EIP712 domain separator.
	bytes32 internal immutable _msgTypeHash; ///@dev eg keccak256("ValidateData(address from,address to,uint256 amount,bytes32 data)");
		...
		return
			keccak256(
				abi.encodePacked(
					"\x19\x01",
					_domainSeparator,
					_getERC1400StructHash(_validateDataParams, nonce, deadline)
				)
			);
	}
		...
		return
			keccak256(
				abi.encodePacked(
					"\x19\x01",
					_domainSeparator,
					_getERC1400NFTStructHash(_validateDataParams, nonce, deadline)
				)
			);
	}
}
