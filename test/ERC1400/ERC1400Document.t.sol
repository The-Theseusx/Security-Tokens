...
contract ERC1400DocumentTest is ERC1400BaseTest {
    ...
    function testSetDocumentFailsWhenCallerNotAdmin() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                AccessControlUnauthorizedAccount.selector, alice, ERC1400MockToken.ERC1400_ADMIN_ROLE()
            )
        );
        ERC1400MockToken.setDocument(documentName, documentURI, documentHash);
        vm.stopPrank();
    }
    ...
    function testSetDocument() public {
        ...
        skip(5 hours);
        (string memory docUri, bytes32 docHash, uint256 lastUpdated) = ERC1400MockToken.getDocument(documentName);
    ...
    function testRemoveDocumentFailsWhenCallerNotAdmin() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                AccessControlUnauthorizedAccount.selector, notTokenAdmin, ERC1400MockToken.ERC1400_ADMIN_ROLE()
            )
        );
        ERC1400MockToken.removeDocument(documentName);
        vm.stopPrank();
    }
    ...
    function testGetAllDocuments() public {
        bytes32[] memory allDocs = ERC1400MockToken.getAllDocuments();
        ...
        ERC1400MockToken.setDocument("Asset6", "https://example.com", keccak256("https://example.com"));
        vm.stopPrank();

        allDocs = ERC1400MockToken.getAllDocuments();
