...
abstract contract ERC1400CanTransferTest is ERC1400BaseTest {
	...
	function testCanTransferFromContracts() public {
		...
		(bool can, bytes memory reason, ) = ERC1400MockToken.canTransferFrom(
			tokenAdmin,
			address(nonERC1400ReceivableContract),
			1000e18,
			""
		...
		assertFalse(
			can,
			"canTransferFrom should return false when to is a contract that does not implement ERC1400Receiver"
		);
		assertEq(reason, INVALID_RECEIVER, "ERC1400: transfer to non ERC1400Receiver implementer should fail");
