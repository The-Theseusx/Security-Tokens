...
import { ERC1644TestStorage } from "./ERC1644TestStorage.sol";

abstract contract ERC1644NFTSigUtils is ERC1644TestStorage {
    ...
    function prepareTransferSignature(
        ...
        bytes32 role = mockERC1644.ERC1594_TRANSFER_AGENT_ROLE();
        nonce = nonce == 0 ? mockERC1644.getRoleNonce(role) : nonce;
    ...
    {
        bytes32 role = mockERC1644.ERC1594_REDEEMER_ROLE();
        nonce = nonce == 0 ? mockERC1644.getRoleNonce(role) : nonce;
        deadline = deadline == 0 ? uint48(block.timestamp + 1 minutes) : deadline;
