...
contract ERC1594NFT is IERC1594NFT, ERC721, EIP712, AccessControl {
	...
	function issue(
		...
		uint256 tokenId,
		bytes calldata data
	) public virtual override onlyRole(ERC1594NFT_ISSUER_ROLE) {
	...
	function redeemFrom(
		...
		uint256 tokenId,
		bytes calldata data
	) public virtual override onlyRole(ERC1594NFT_REDEEMER_ROLE) {
	...
	function _validateData(
		...
		uint256 tokenId,
		bytes calldata data
	) internal view virtual returns (bool, address authorizer) {
		...
		bytes32 structData = keccak256(
			abi.encodePacked(ERC1594NFT_DATA_VALIDATION_HASH, from, to, tokenId, _roleNonce[authorizerRole_], deadline)
		);
